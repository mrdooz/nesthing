...
void version(void) {
 fprintf(stderr,"DCC6502 %s (C)1998-99 Tennessee Carmel-Veilleux\n",VERSION_INFO);
 fprintf(stderr,"This is free software. To see the LICENSE, use the -v parameter\n");
// fprintf(stderr,"This program was built on %s with GCC %s\n\n",__DATE__,__VERSION__);
}
...
void license(void) {
 ...
 fprintf(stderr,"in the universe (not excluding deities). That means: enjoy it\n");
 fprintf(stderr,"and don't complain :)\n");
}
...
int main(int argc, char *argv[]) {
 ...
 if (argc > 2) {
  for (i = 1; i < argc - 1; i++) {
   ...
   switch (argv[i][1]) {
    case '?': version(); usage(); license(); exit(0); break;
    case 'n': nes_mode = 1; break;
    case 'c': cycle_counting = 1; break;
    case 'h': hex_output = 1; break;
    case 'v': version(); license(); exit(0); break;
